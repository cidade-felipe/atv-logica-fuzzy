...
def copiar_assets_relatorio(caminho_saida: Path) -> dict[str, Path]:
    css_origem = WEB_ASSETS_DIR / 'css' / 'previsao_humor.css'
    js_origem = WEB_ASSETS_DIR / 'js' / 'previsao_humor.js'

    assets_dir = caminho_saida.parent / 'assets'
    css_dir = assets_dir / 'css'
    js_dir = assets_dir / 'js'
    css_dir.mkdir(parents=True, exist_ok=True)
    js_dir.mkdir(parents=True, exist_ok=True)

    css_destino = css_dir / css_origem.name
    js_destino = js_dir / js_origem.name
    dados_js_destino = js_dir / 'previsao_humor_dados.js'

    shutil.copyfile(css_origem, css_destino)
    shutil.copyfile(js_origem, js_destino)

    return {
        'css': css_destino,
        'js': js_destino,
        'dados_js': dados_js_destino,
    }


def salvar_dados_graficos_js(
    grafico_pertinencia: go.Figure,
    grafico_resultados: go.Figure,
    caminho_saida: Path,
) -> None:
    figuras = {
        'pertinencia': json.loads(pio.to_json(grafico_pertinencia)),
        'resultados': json.loads(pio.to_json(grafico_resultados)),
    }

    conteudo = 'window.previsaoHumorFiguras = '
    conteudo += json.dumps(figuras, ensure_ascii=False, indent=2)
    conteudo += ';\n'
...
def gerar_relatorio_html(
    ...
    assets = copiar_assets_relatorio(caminho_saida)

    grafico_pertinencia = criar_grafico_pertinencia(variaveis)
    grafico_resultados = criar_grafico_resultados(resultados)
    salvar_dados_graficos_js(grafico_pertinencia, grafico_resultados, assets['dados_js'])

    linhas_tabela = '\n'.join(
        (
            f'<tr data-categoria="{resultado.categoria}">'
...
<head>
  ...
  <link rel="stylesheet" href="assets/css/previsao_humor.css">
</head>
<body>
  ...
  <section aria-labelledby="titulo-grafico-resultados">
    <h2 id="titulo-grafico-resultados">Resultado por perfil</h2>
    <div id="grafico-resultados" class="grafico"></div>
  </section>
  <section aria-labelledby="titulo-grafico-pertinencia">
    <h2 id="titulo-grafico-pertinencia">Funcoes de pertinencia</h2>
    <div id="grafico-pertinencia" class="grafico"></div>
  </section>
  <script src="https://cdn.plot.ly/plotly-latest.min.js"></script>
  <script src="assets/js/previsao_humor_dados.js"></script>
  <script src="assets/js/previsao_humor.js"></script>
</body>
</html>
"""
